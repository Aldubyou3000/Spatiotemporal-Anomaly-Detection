...
def convert_temp(value, detected_unit, display_unit):
    """Convert a single temperature value for display.

    Handles full conversion logic including mapping display_unit to actual units.
    Preserves NaN values.

    Args:
        value: Single numeric temperature value (or NaN)
        detected_unit: 'C' or 'F' (original data unit)
        display_unit: 'Original', 'Celsius', or 'Fahrenheit' (desired display)

    Returns:
        Converted float value, or NaN if input is NaN

    Example:
        >>> convert_temp(28.5, 'C', 'Fahrenheit')
        83.3
        >>> convert_temp(28.5, 'C', 'Original')
        28.5
    """
    if pd.isna(value):
        return np.nan
...
def convert_temp_series(series, detected_unit, display_unit):
    ...
    return series.apply(
        lambda x: convert_temperature(
            x, from_unit, to_unit) if pd.notna(x) else np.nan
    )
