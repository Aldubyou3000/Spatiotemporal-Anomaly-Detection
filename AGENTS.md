...
- **`Card` / `CardHeader` / `CardTitle` / `CardBody`** — surface card with dividers (available primitive, currently unused — pages build cards inline)
...
EXPO_PUBLIC_OAUTH_URL=          # blank → falls back to EXPO_PUBLIC_API_URL; set to the
                                # Vercel first-party host for EAS/production OAuth
```
**EAS builds ignore `App/.env`** — `App/eas.json` bakes `EXPO_PUBLIC_API_URL` (Render)
and `EXPO_PUBLIC_OAUTH_URL` (Vercel first-party proxy) per profile. Google OAuth must
run on a clean first-party origin (`…vercel.app`), never directly on the shared
`onrender.com` host (Chrome Safe Browsing block) — see `docs/KNOWN_BUGS_AND_FIXES.md`.
