...
| File | Role |
|------|------|
| `routers/zones.py` | HTTP layer: `require_analyst`, accepts `UploadFile`s, calls `run_pipeline_multi` via `run_in_threadpool`, raises `ZoneProcessingError` → 400. |
...
| `zones/zone_c.py` | **Frozen.** LOF (RobustScaler, 1D rainfall feature, threshold=1.5). |
| `schemas/zones.py` | `ProcessResult` and supporting Pydantic models. |

## Depends on
- `core/dependencies.py` → `require_analyst`
- `zones/zone_a|b|c.py`
...
## Key invariants
- **Zone algorithms are frozen.** `api/app/zones/` is the canonical source of truth — no other copy exists in the repo. Do not modify `zone_a/b/c.py`.
- The pipeline is synchronous from the client's perspective but runs in a threadpool on the server. Long runs hold a worker thread.
