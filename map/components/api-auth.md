...
Password + Google OAuth (server-side PKCE) for the **web** analyst dashboard. Mobile has its own parallel endpoints under `routers/mobile.py`. Issues httpOnly cookies (access 30m, refresh 7d), CSRF cookie, and a session-fingerprint cookie.

## What it does
- `POST /api/auth/login` — verify credentials against Supabase Auth, enforce `lockout`, issue cookie triplet + CSRF, log `LOGIN_SUCCESS`/`LOGIN_FAILED`/`LOGIN_LOCKED`.
- `GET /api/auth/me` — current profile.
- `POST /api/auth/logout`, `POST /api/auth/refresh` — rotate tokens; refresh rotates the refresh token itself (anti-reuse).
- Google OAuth: `GET /api/auth/oauth/google/start` → 302 to Google; `GET /api/auth/oauth/google/callback/{state}` → exchange, enforce analyst role, set cookies, redirect to `WEB_APP_URL` (`/zones`).
